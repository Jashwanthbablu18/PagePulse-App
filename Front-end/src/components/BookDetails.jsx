// client/src/components/BookDetails.jsx
import { useEffect, useState } from 'react'
import { Link } from 'react-router-dom'
import { getBook, getInsights } from '../utils/api'
import Description from "./Description"
import { SkewLoader } from 'react-spinners' // ⬅️ add
...
export default function BookDetails({ workId }) {
  const [bundle, setBundle] = useState(null) // { book, author, buyLinks, moreFromAuthor, ai, ai_error }
  ...
  // 🔄 Show gradient SkewLoader while backend content is loading
  if (loading) {
    return (
      <div
      ...
      >
        <div className="position-relative" style={{ width: 64, height: 40 }}>
          {/* Overlap three SkewLoaders to emulate a smooth gradient */}
        ...
        <div className="col-12 col-md-4">
          <div className="card border-0 shadow-sm rounded-4 p-3 text-center">
            {book.thumbnail ? (
              ...
              <div className="bg-light rounded-4 d-flex align-items-center justify-content-center" style={{ height: 320 }}>No cover</div>
            )}
            <div className="mt-3">
          ...
          <div className="card border-0 shadow-sm rounded-4">
            <div className="card-body">
              ...
              <div className="row g-3 mb-3">
                {book.publishedDate && <div className="col-6 col-lg-4"><div className="small text-muted">Published</div><div>{book.publishedDate}</div></div>}
                {book.publisher && <div className="col-6 col-lg-4"><div className="small text-muted">Publisher</div><div>{book.publisher}</div></div>}
                {book.language && <div className="col-6 col-lg-4"><div className="small text-muted">Language</div><div>{book.language}</div></div>}
              </div>
